...
exports.createChallenge = catchAsync(async (req, res, next) => {
    ...
    if (!Array.isArray(exerciseIds) || exerciseIds.length === 0) return next(
        new AppError('exerciseIds must be a non-empty array', 400)
    );
...
exports.getChallenge = catchAsync(async (req, res, next) => {
    ...
    if (joinCode) {
        ...
        return next(
            new AppError('Challenge identifier is required', 400)
        );
    }
    ...
    if (!challenge) {
        return next(
            new AppError('Challenge not found', 404)
        );
    }

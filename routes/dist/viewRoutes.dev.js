...
router.get('/reset-password/:token', viewController.resetPassword); // router.use(authController.protect); // Do not put this here cuz it will 
// also protect the unknwon routes e.g. /sdafasdf then it will say 
// 'You are not logged in. Please log in' we don't want that.

router.get("/dashboard", authController.protect, authController.restrictTo("user"), adminController.getGymspireNowStatus, viewController.dashboard);
router.get("/adminDashboard", authController.protect, authController.restrictTo("admin"), viewController.adminDashboard);
router.get("/coachDashboard", authController.protect, authController.restrictTo("coach"), viewController.coachDashboard);
router.get("/profile", authController.protect, viewController.profile);
router.get("/workoutPlan", authController.protect, viewController.workoutPlan);

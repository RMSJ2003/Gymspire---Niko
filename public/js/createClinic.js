const form = document.querySelector("#createClinicForm");
...
form.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    const res = await fetch("/api/v1/admin/createClinic", {
      method: "POST",
    ...
    if (!res.ok) {
      throw new Error(data.message || "Create clinic failed");
    }

    // 🔥 SUCCESS UI
    success = true;
    formMessage.textContent = "Clinic created successfully! Redirecting...";
    submitBtn.disabled = true;
    ...
    const message = err.message || "Create clinic failed";
    ...
    if (!success) {
      ...
      btnText.textContent = "Create Clinic";
    }
  }
});
